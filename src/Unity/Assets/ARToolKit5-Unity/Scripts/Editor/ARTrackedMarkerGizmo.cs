...
class ARTrackedMarkerGizmo {
...
#endif
	private static void RenderARTrackedMarkerGizmo(ARTrackedMarker arTrackedMarker, GizmoType gizmoType) {
		if (null == arTrackedMarker || !arTrackedMarker.gameObject.activeInHierarchy) {
		...
		if (arTrackedMarker.UID == ARMarker.NO_ID) {
			arTrackedMarker.Load();
		}

		Matrix4x4 pose = arTrackedMarker.gameObject.transform.localToWorldMatrix;

        switch (arTrackedMarker.MarkerType) {

            case MarkerType.Square:
            case MarkerType.SquareBarcode:
				DrawSingleMarker(arTrackedMarker, pose, selected);
                break;

			case MarkerType.Multimarker:
				DrawMultiMarker(arTrackedMarker, pose, selected);
			    break;
			
			case MarkerType.NFT:
				DrawNFTMarker(arTrackedMarker, pose, selected);
			    break;

        }
    }

	private static void DrawSingleMarker(ARTrackedMarker arTrackedMarker, Matrix4x4 mat, bool selected) {
        float pattWidth = arTrackedMarker.PatternWidth;
		...
		DrawWord(arTrackedMarker.EditorMarkerName, wordUnitSize, origin - up * (pattWidth * 0.6f + (wordUnitSize * 4)) - right * (pattWidth * 0.525f), up, right * 0.5f);
    }
	...
	private static void DrawMultiMarker(ARTrackedMarker arTrackedMarker, Matrix4x4 mat, bool selected) {
		for (int i = 0; i < arTrackedMarker.Patterns.Length; ++i) {
			...
			float wordUnitSize = pattWidth * 0.02f;
			DrawWord(arTrackedMarker.EditorMarkerName + "(" + i + ")", wordUnitSize, origin - up * (pattWidth * 0.6f + (wordUnitSize * 4)) - right * (pattWidth * 0.525f), up, right * 0.5f);
		}
    }

	private static void DrawNFTMarker(ARTrackedMarker arTrackedMarker, Matrix4x4 mat, bool selected) {
        float pattWidth  = arTrackedMarker.NFTWidth;
		float pattHeight = arTrackedMarker.NFTHeight;
		if (pattWidth > 0.0f && pattHeight > 0.0f) {
			...
			DrawWord(arTrackedMarker.EditorMarkerName, wordUnitSize, centre - up*(pattHeight*0.6f + (wordUnitSize*4)) - right*pattWidth*0.525f, up, right*0.5f);
		}
	}
