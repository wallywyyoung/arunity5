...
public class ARPattern {
    ...
    public ARPattern(int markerID, int patternID) {
		...
		width  = widthRaw  * ARTrackedMarker.UNITY_TO_ARTOOLKIT;
		height = heightRaw * ARTrackedMarker.UNITY_TO_ARTOOLKIT;
		// Scale the position from ARToolKit units (mm) into Unity units (m).
		matrixRawArray[12] *= ARTrackedMarker.UNITY_TO_ARTOOLKIT;
		matrixRawArray[13] *= ARTrackedMarker.UNITY_TO_ARTOOLKIT;
		matrixRawArray[14] *= ARTrackedMarker.UNITY_TO_ARTOOLKIT;
